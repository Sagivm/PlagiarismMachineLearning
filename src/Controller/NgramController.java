...
public class NgramController {
		...
		for (int i = 0; i < ngrams.size(); i++) {
			...
			for (int j = 0; j < ngram.getData().size(); j++) {
				...
				if (index == -1)// if general contains term
					...
					generalNgram.getData().get(index)
							.setFrequency(generalNgram.getData().get(index).getFrequncy() + term.getFrequncy());
			}
		}
	}
	...
	public static int getNumberofNgrams() {
