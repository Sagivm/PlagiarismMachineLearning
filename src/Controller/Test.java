...
public class Test {
	static String[] stopWords = { "without", "see", "unless", "due", "also", "must", "might", "like", "]", "[", "}",
			...
			"most", "mustn’t", "my", "myself", "no", "nor", "not", "of", "off", "on", "once", "only", "ought", "our",
	...
	public static void main(String[] args) {
		...
		/*Test t = new Test();
		System.out.println(t.remover("I am loving hating investment flyer buyer bought created"));
		t.divideIntoChunks("hellomynameisguyzionbay",4);*/

	}

	public boolean contains(String[] s, String w) {
		for (int i = 0; i < s.length; i++)
			if (s[i].toLowerCase().equals(w.toLowerCase()))
				return true;
		return false;
	}

	//removing stop words, spaces, and stemming.
	public String remover(String s) {
		...
		for (int i = 0; i < arr.length; i++) {
			stem.add(arr[i].toCharArray(), arr[i].length());
			stem.stem();
			char[] ss = stem.getResultBuffer();
			String temp = "";
			for (int j = 0; j < stem.getResultLength(); j++)
				temp = temp + ss[j];
			res = res + temp;
		}
		return res;
	...
	public ArrayList<String> divideIntoChunks(String s, int k){
